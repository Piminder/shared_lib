...
export const is_broadcast_store_payment_data = (
  data: any,
): data is BroadcastStorePaymentData =>
...
export default class Listener {
  ...
  public on(callback: CallBack) {
    ...
    // socket connections
    this.io.on("connect", () => {
      MorgansWrapper.log("Listener credit service, with id:", this.io.id);
    });

    this.io.on("connect_error", (error) => {
      MorgansWrapper.err("Erro de conexão:", error.message);
    });

    this.io.on("disconnect", () => {
      MorgansWrapper.log("Desconectado do servidor");
    });
  }
