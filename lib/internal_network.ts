...
export default class InternalServiceNetwork {
	...
	public async get_installment_to_notify(): Promise<Result<unknown[]>> {
		...
		try {
			...
			console.error(
				`1. Error when geting installment to notify: ${error.response.data.message}`,
			);
			console.error(`2. Error when geting installments to notify: ${error}`);
	...
	public async get_public_product_byref(
		id: string,
	): Promise<Result<IPublicProductMessage>> {
		const headers = {
			"Content-Type": "application/json",
		};
	...
	public async delete_internal_cus_in_invoice_db(
		cus_id: string,
	): Promise<Result<void>> {
		const headers = {
			"Content-Type": "application/json",
